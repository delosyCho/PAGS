...
class Improved_AoA_Reader:
    ...
    def model_Index(self):
        ...
        with tf.variable_scope("decoding") as scope:
            ...
            Prediction = tf.reshape(Prediction_, shape=[self.batch, self.p_length])
    ...
    def model(self):
        ...
        with tf.variable_scope("attention") as scope:
            ...
            S_Vector = tf.reshape(S_Vector, shape=[self.batch, self.p_length])
    ...
    def training_prediction_index(self, training_epoch, is_continue, is_Start = True):
        with tf.Session() as sess:
            ...
            while self.dataset.whole_batch_index < training_epoch:
                ...
                training_feed_dict = {tensor_index: label, self.x_pos_holder: self.POS_Embeddings, self.x_p_holer: self.paragraph, self.x_q_holer: self.question}
    ...
    def test_index(self, is_Start = True):
        with tf.Session() as sess:
            ...
            for a in range(self.batch):
                ...
                for i in range(25):
                    # st = st + str(self.question[a, i]) + ' '
                    st = st + self.dataset.paragraph_arr[self.dataset.paragraph_index[a], i] + ','
                print(st)
    ...
    def getProp_Start_Stop_Index(self, paragraph_, question_, is_Test=True):
        ...
        if is_Test:
            paragraph_, question_, self.start_index, self.stop_index, self.POS_Embeddings = self.dataset.get_test_batch()
